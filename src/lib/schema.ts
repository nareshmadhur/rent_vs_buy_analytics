...
export const analysisSchema = z.object({
  ...
  maintenancePercentage: z.coerce.number({ required_error: "Maintenance is required." }).nonnegative({ message: "Percentage must be non-negative." }).max(10, { message: "Percentage seems too high." }).default(1),
  ...
  estimatedSellingCostsPercentage: z.coerce.number({ required_error: "Selling costs are required."}).nonnegative().max(10).default(2),
